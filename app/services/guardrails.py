...
def _rotate_copy(options: list[str]) -> str:
    if not options:
        return ""
    return options[datetime.utcnow().minute % len(options)]


def build_greeting_response() -> str:
    return _rotate_copy(
        [
            "سلام عزیزم 😊 خوش اومدی به قلب دوم. دنبال چی هستی؟",
            "سلام 🌷 خوش اومدی. بگو چه مدل یا چه بازه قیمتی مدنظرته تا سریع پیشنهاد بدم.",
            "درود ✨ خوشحال می‌شم کمک کنم؛ دنبال چه محصولی هستی؟",
        ]
    )
...
def build_thanks_response() -> str:
    return _rotate_copy(
        [
            "خواهش می‌کنم! اگر سوالی داشتید در خدمتم.",
            "باعث افتخاره 🌷 هر زمان سوالی بود، من هستم.",
            "خوشحالم کمک کردم. اگر بخواید گزینه‌های بیشتری هم معرفی می‌کنم.",
        ]
    )
...
def build_decline_response() -> str:
    return _rotate_copy(
        [
            "باشه، هر وقت سوالی داشتید خوشحال می‌شم کمک کنم.",
            "چشم، هر زمان بخواید دوباره راهنمایی‌تون می‌کنم.",
            "اوکی 👌 اگر بعداً مدل خاصی خواستید پیام بدید.",
        ]
    )
...
def build_goodbye_response() -> str:
    return _rotate_copy(
        [
            "روز خوبی داشته باشید! هر وقت سوالی بود پیام بدید.",
            "وقتتون بخیر 🌷 هر زمان نیاز داشتید در خدمتیم.",
            "خدانگهدار، امیدوارم انتخاب خوبی داشته باشید ✨",
        ]
    )
